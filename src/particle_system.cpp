...
void ParticleSystem::Draw() {
  for (Particle& particle : particle_pool_) {
    ...
    // Decay particles.
    float life = particle.life_time / particle.life_remaining;
    glm::vec4 colour =
        glm::lerp(particle.colour_begin, particle.colour_end, life);
    colour.a *= life;

    float size = glm::lerp(particle.size_begin, particle.size_end, life);

    // Calculate model matrix.
    glm::mat4 model_mat(1.0f);
    model_mat = glm::translate(model_mat, particle.position);
    model_mat =
        glm::rotate(model_mat, particle.rotation, glm::vec3(1.0f, 1.0f, 1.0f));
    ...
    renderer_.Draw(vertex_array_object_, shader_, vertices_.size());
  }
}
